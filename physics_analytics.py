...
if __name__ == "__main__":
    ...
    # Let's analyze a stable orbit first
    sim_id_to_test = 0
    ...
    # Arrays to store the physics data
    ...
    # 2. Extract AI's Learned Physics Parameters
    k_ai = model.k.item()
    gamma_ai = model.gamma.item()

    # 3. Energy Calculations (Assuming mass = 1)
    KE = 0.5 * v2
    # The integral of Force (-k/r^gamma) gives Potential Energy: U = -k / ((gamma-1) * r^(gamma-1))
    PE = -k_ai / ((gamma_ai - 1.0) * (r ** (gamma_ai - 1.0)))
    TE = KE + PE

    # 4. Angular Momentum (L = r x v)
    L = x * vy - y * vx
    ...
    # Shade the Escape Velocity Zone
    ...
    # Calculate how perfectly flat the line is (Variance)
    l_variance = np.var(L)
